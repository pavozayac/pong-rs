...
impl App {
    fn render(&mut self, args: &RenderArgs, gc: &mut GlyphCache) {
        ...
        self.gl.draw(args.viewport(), |c, gl| {
            ...
            rectangle(WHITE, paddle, transform_right, gl);
    ...
    fn update(&mut self, args: &UpdateArgs) {
        ...
        if self.pong.pos.x >= self.window_size[0] - PADDING - PADDLE_WIDTH {
            if self.pong.pos.y >= self.right_y && self.pong.pos.y <= self.right_y + PADDLE_HEIGHT {
                self.pong.vel = Matrix2::new(-1.0, 0.0, 0.0, 1.0) * self.pong.vel;
